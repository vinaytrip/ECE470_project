...
def main():
    ...
    sub_gripper = rospy.Subscriber("ur3/gripper_input", gripper_input, topic_callback)
    ...
    while(not input_done):
        input_string = raw_input("Start press 1: ")
    ...
    while(loop_count > 0):
        ...
        move_arm(pub_command, loop_rate, Q[0][2][0], 4.0, 4.0)
        ...
        time.sleep(1.0)
        move_arm(pub_command, loop_rate, Q[0][2][1], 4.0, 4.0)
        ...
        move_arm(pub_command, loop_rate, home, 4.0, 4.0)
        move_arm(pub_command, loop_rate, Q[0][1][0], 4.0, 4.0)
...
# Delay to make sure suction cup has grasped the block
        time.sleep(1.0)
        move_arm(pub_command, loop_rate, Q[0][1][1], 4.0, 4.0)
        gripper(pub_command, loop_rate, suction_off)
        if(digin_0):
            print("Gripper is on")
        else:
